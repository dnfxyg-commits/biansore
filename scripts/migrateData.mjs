...
async function migrateData() {
  ...
  const exhibitionsData = EXHIBITIONS.map(item => ({
    id: item.id, // Using string ID
    title: item.title,
    ...
    lat: item.coordinates.lat, // Flatten coordinates
    lng: item.coordinates.lng,
    region: item.region,
    website_url: item.websiteUrl
  }));
  
  const { error: exError } = await supabase.from('exhibitions').insert(exhibitionsData);
  if (exError) console.error('Error migrating exhibitions:', exError);
  else console.log('Exhibitions migrated successfully.');

  // 2. Exhibition Centers
  console.log('Migrating Exhibition Centers...');
  const centersData = EXHIBITION_CENTERS.map(item => ({
    id: item.id,
    name: item.name,
    name_en: item.nameEn,
    location: item.location,
  ...
  const { error: centerError } = await supabase.from('exhibition_centers').insert(centersData);
  if (centerError) console.error('Error migrating exhibition centers:', centerError);
  else console.log('Exhibition Centers migrated successfully.');
  ...
  const { error: blogError } = await supabase.from('blog_posts').insert(blogsData);
  if (blogError) console.error('Error migrating blog posts:', blogError);
  else console.log('Blog Posts migrated successfully.');
